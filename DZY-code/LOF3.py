...
res = []
n_neighborss = [10,20,50,100,200]
contaminations = [float(0.1),float(0.2),'auto']
step = 0
for n_neighbors in n_neighborss:
    for contamination in contaminations:
        ...
        res.append({'次数': step + 1,'k邻近距离': n_neighbors, '异常值比例': contamination,
...
# 将迭代后的结果存储到数据框中
df = pd.DataFrame(res)
df.to_excel('data_LOF_3.xls',sheet_name='LOF_3')
print(df)
